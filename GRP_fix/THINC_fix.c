 ...
 double rho[], double u[], double p[], runHist *runhist, char *scheme)
{
  ...
  running_info[1] = 0.0;
  running_info[1] = 0.0;
  THINC_primitive_0(running_info, m, h, thickness, rho, u, p, rho_L, rho_R, u_L, u_R, p_L, p_R, D_rho_L, D_rho_R, D_u_L, D_u_R, D_p_L, D_p_R, trouble);
//------------THE MAIN LOOP-------------
  for(k = 1; k <= MaxStp; ++k)
  {
    if(T+eps > TIME)
