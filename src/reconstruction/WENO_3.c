 ...
 double D_rho_L[], double D_rho_R[], double D_u_L[], double D_u_R[], double D_p_L[], double D_p_R[])
{
  ...
  if(bod < 0)
  {
    ...
    W1[m+hl] = rho[m-1]; W1[m+hl+1] = rho[m-2];
    W2[m+hl] =-mom[m-1]; W2[m+hl+1] =-mom[m-2];
    W3[m+hl] = ene[m-1]; W3[m+hl+1] = ene[m-2];
  }
  else if(bod > 0)
  {
    ...
    W1[m+hl] = rho[m-1]; W1[m+hl+1] = rho[m-1];
    W2[m+hl] = mom[m-1]; W2[m+hl+1] = mom[m-1];
    W3[m+hl] = ene[m-1]; W3[m+hl+1] = ene[m-1];
  }
  else
  {
    ...
    W1[m+hl] = rho[0];  W1[m+hl+1] = rho[1];
    W2[m+hl] = mom[0];  W2[m+hl+1] = mom[1];
    W3[m+hl] = ene[0];  W3[m+hl+1] = ene[1];
  }
  ...
  //=========Charactoristic Decomposition=========
    decomposition(H_star, u_star, c_square, c_star, gamma1, m, j, l, hl, decomp, W1, W2, W3, Q1, Q2, Q3);
  ...
  //=====Recomposition========
    ...
    rho_L[j-hl] = PL[0]; rho_R[j-hl] = PR[0];
    u_L[j-hl] = PL[1];   u_R[j-hl] = PR[1];
    p_L[j-hl] = PL[2];   p_R[j-hl] = PR[2];
    QL[0] = DQQ[0][0]; QR[0] = DQQ[0][1];
    QL[1] = DQQ[1][0]; QR[1] = DQQ[1][1];
    QL[2] = DQQ[2][0]; QR[2] = DQQ[2][1];
    recomposition(H_star, u_star, c_star, decomp, QL, QR, PL, PR);
    D_rho_L[j-hl] = PL[0]; D_rho_R[j-hl] = PR[0];
    D_u_L[j-hl] = PL[1];   D_u_R[j-hl] = PR[1];
    D_p_L[j-hl] = PL[2];   D_p_R[j-hl] = PR[2];

    u_L[j-hl] = u_L[j-hl] / rho_L[j-hl];
    u_R[j-hl] = u_R[j-hl] / rho_R[j-hl];
    p_L[j-hl] = (p_L[j-hl] - 0.5*u_L[j-hl]*u_L[j-hl]*rho_L[j-hl])*(gamma-1.0);
    p_R[j-hl] = (p_R[j-hl] - 0.5*u_R[j-hl]*u_R[j-hl]*rho_R[j-hl])*(gamma-1.0);

    D_u_L[j-hl] = (D_u_L[j-hl] - u_L[j-hl]*D_rho_L[j-hl]) / rho_L[j-hl];
    D_u_R[j-hl] = (D_u_R[j-hl] - u_R[j-hl]*D_rho_R[j-hl]) / rho_R[j-hl];
    D_p_L[j-hl] = (D_p_L[j-hl] - 0.5*D_rho_L[j-hl]*u_L[j-hl]*u_L[j-hl] - rho_L[j-hl]*u_L[j-hl]*D_u_L[j-hl])*(gamma-1.0);
    D_p_R[j-hl] = (D_p_R[j-hl] - 0.5*D_rho_R[j-hl]*u_R[j-hl]*u_R[j-hl] - rho_R[j-hl]*u_R[j-hl]*D_u_R[j-hl])*(gamma-1.0);
  }
}
