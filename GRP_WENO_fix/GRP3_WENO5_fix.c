...
//------------THE MAIN LOOP-------------
  ...
  {    
    ...
    */
    ...
    WENO_50(running_info, m, h, eps, alp2, gamma, half_rho, half_mom, half_ene, rho_L, rho_R, u_L, u_R, p_L, p_R, D_rho_L, D_rho_R, D_u_L, D_u_R, D_p_L, D_p_R);
    ...
    for(j = 0; j < m+1; ++j)
    {
      ...
      F1[j] = (2.0*f01[j] + f11[j] + 0.5*tau*g01[j])/3.0;
      F2[j] = (2.0*f02[j] + f12[j] + 0.5*tau*g02[j])/3.0;
      F3[j] = (2.0*f03[j] + f13[j] + 0.5*tau*g03[j])/3.0;
