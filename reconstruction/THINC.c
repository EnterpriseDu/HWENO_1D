 ...
 double D_rho_L[], double D_rho_R[], double D_u_L[], double D_u_R[], double D_p_L[], double D_p_R[], int trouble[])
{
  ...
  double u_min, u_max, u_bar, result[4];
 ...
 double D_rho_L[], double D_rho_R[], double D_u_L[], double D_u_R[], double D_p_L[], double D_p_R[], int trouble[])
{
  ...
  if(bod < 0)
  {
    P1[0]   = rho[0];  P2[0]   =-mom[0];  P3[0]   = ene[0];
    P1[m+1] = rho[m-1];P2[m+1] =-mom[m-1];P3[m+1] = ene[m-1];
  }
  else if(bod > 0)
  {
    P1[0]   = rho[0];  P2[0]   = mom[0];  P3[0]   = ene[0];
  ...
  {
    P1[0]   = rho[m-1];P2[0]   = mom[m-1];P3[0]   = ene[m-1];
    P1[m+1] = rho[0];  P2[m+1] = mom[0];  P3[m+1] = ene[0];
  }
  ...
  {
    u_L[j] = u_L[j]/rho_L[j];
    p_L[j] = gamma1*(p_L[j] - 0.5*rho_L[j]*u_L[j]*u_L[j]);
    u_R[j] = u_R[j]/rho_R[j];
    p_R[j] = gamma1*(p_R[j] - 0.5*rho_R[j]*u_R[j]*u_R[j]);
    D_u_L[j] = (D_u_L[j] - D_rho_L[j]*u_L[j]) / rho_L[j];
    D_p_L[j] = gamma1*(D_p_L[j] - 0.5*D_rho_L[j]*u_L[j]*u_L[j] - rho_L[j]*u_L[j]*D_u_L[j]);
    D_u_R[j] = (D_u_R[j] - D_rho_R[j]*u_R[j]) / rho_R[j];
    D_p_R[j] = gamma1*(D_p_R[j] - 0.5*D_rho_R[j]*u_R[j]*u_R[j] - rho_R[j]*u_R[j]*D_u_R[j]);
  }
}
